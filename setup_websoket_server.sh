...
# === Configuration ===
APP_DIR="$HOME/websocket_server"
VENV_NAME="server_middleman"
VENV_DIR="$APP_DIR/$VENV_NAME"
APP_FILE="websocket_server.py"

echo "🔧 Setting up WebSocket server in $APP_DIR"
...
async def main():
    ...
    app.add_routes([
        ...
        web.get('/pi_control', handle_pi_control),
        web.get('/pi_distance', handle_pi_distance),
        web.static('/', os.path.join(os.getcwd(), 'static'))
    ])
    logger.info("WebSocket server started on http://0.0.0.0:9000")
    return app
...
SERVICE_FILE="/etc/systemd/system/websocket_server.service"
...
[Service]
ExecStart=$VENV_DIR/bin/python $APP_DIR/$APP_FILE
...
echo "✅ WebSocket server is running on port 9000 using virtual environment '$VENV_NAME'"